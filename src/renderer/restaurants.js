document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function refresh() {
    ...
    // Si admin/cuisinier, ajouter bouton d'ajout rapide
    if (user.roles.includes('Admin') || user.roles.includes('Cuisinier')) {
      ...
      addForm.addEventListener('submit', async (e) => {
        ...
        if (!nom) { showMessage(detailEl, 'Le nom est requis.', 'error'); return; }
        btn.disabled = true;
        try {
          const res = await window.api.addRestaurant(user.id, { nom, adresse, telephone });
          if (res && res.success) { showMessage(detailEl, 'Restaurant ajouté !', 'success'); addForm.reset(); refresh(); }
          else showMessage(detailEl, 'Erreur lors de l\'ajout: '+(res && res.error ? res.error : ''), 'error');
        } catch (err) { showMessage(detailEl, 'Erreur réseau.', 'error'); }
        btn.disabled = false;
      });
  ...
  function renderSelectedRestaurant(){
    ...
    // Afficher les sections avec leurs produits
  ...
  function attachDetailEvents(){
    ...
    detailEl.querySelectorAll('.del-resto').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        if (res && res.success) { showMessage(detailEl, 'Restaurant supprimé.', 'success'); refresh(); }
        else { showMessage(detailEl, 'Erreur suppression.', 'error'); btn.disabled = false; }
      });
    });
    ...
    detailEl.querySelectorAll('.edit-resto').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        if (res && res.success) { showMessage(detailEl, 'Restaurant modifié.', 'success'); refresh(); }
        else { showMessage(detailEl, 'Erreur modification.', 'error'); btn.disabled = false; }
      });
    });
    ...
    detailEl.querySelectorAll('.del-prod').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        if (res && res.success) { showMessage(detailEl, 'Produit supprimé.', 'success'); refresh(); }
        else { showMessage(detailEl, 'Erreur suppression produit.', 'error'); btn.disabled = false; }
      });
    });
    ...
    detailEl.querySelectorAll('.edit-prod').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        if (!nom || !prix) { showMessage(detailEl, 'Nom et prix requis.', 'error'); return; }
        btn.disabled = true;
        ...
        if (res && res.success) { showMessage(detailEl, 'Produit modifié.', 'success'); refresh(); }
        else { showMessage(detailEl, 'Erreur modification produit.', 'error'); btn.disabled = false; }
      });
    });
    
    // Ajouter produit dans une section
    detailEl.querySelectorAll('.add-prod-btn').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        if (!prix || isNaN(parseFloat(prix))) { showMessage(detailEl, 'Prix valide requis.', 'error'); return; }
        ...
        if (res && res.success) { showMessage(detailEl, 'Produit ajouté.', 'success'); refresh(); }
        else { showMessage(detailEl, 'Erreur ajout produit.', 'error'); btn.disabled = false; }
      });
    });
    ...
    detailEl.querySelectorAll('.add-section-form').forEach(form => {
      form.addEventListener('submit', async (e) => {
        ...
        if (!nom) { showMessage(detailEl, 'Le nom de la section est requis.', 'error'); return; }
        btn.disabled = true;
        
        const res = await window.api.addSection(user.id, restoId, { nom, description: description || null });
        if (res && res.success) { showMessage(detailEl, 'Section ajoutée.', 'success'); form.reset(); refresh(); }
        else { showMessage(detailEl, 'Erreur ajout section.', 'error'); btn.disabled = false; }
      });
    });
    ...
    detailEl.querySelectorAll('.edit-section').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        if (res && res.success) { showMessage(detailEl, 'Section modifiée.', 'success'); refresh(); }
        else { showMessage(detailEl, 'Erreur modification section.', 'error'); btn.disabled = false; }
      });
    });
    ...
    detailEl.querySelectorAll('.del-section').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        if (res && res.success) { showMessage(detailEl, 'Section supprimée.', 'success'); refresh(); }
        else { showMessage(detailEl, 'Erreur suppression: '+(res.error || ''), 'error'); btn.disabled = false; }
      });
    });
    ...
    detailEl.querySelectorAll('.order-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        // Chercher le produit dans les sections
        let prod = null;
        ...
        if (!prod) { showMessage(detailEl, 'Produit introuvable', 'error'); return; }
  ...
  async function startPaymentFlow(){
    if (cart.length===0) { showMessage(detailEl, 'Panier vide.', 'error'); return; }
