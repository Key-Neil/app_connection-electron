document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    const mineDiv = document.createElement('div'); mineDiv.innerHTML = '<h3>Mes livraisons</h3>';
    if (!livraisons || livraisons.length===0) { mineDiv.innerHTML += '<div>Aucune livraison.</div>'; }
    else {
    ...
    listDiv.appendChild(availDiv); listDiv.appendChild(mineDiv);
    // claim buttons
    listDiv.querySelectorAll('.claim-btn').forEach(btn=>{
      btn.addEventListener('click', async ()=>{
        btn.disabled = true;
        const res = await window.api.createLivraison(user.id, btn.dataset.id);
        if (res && res.success) { alert('Livraison assignée.'); location.reload(); } else { alert('Erreur: '+(res && res.error?res.error:'')); btn.disabled = false; }
      });
    });
